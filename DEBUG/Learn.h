...
void Learn(datatype LayerVec, datatype WeightVec, datatype BiasVec, datatype PreActVec, datatype DLayerVec, datatype WGradVec, datatype BGradVec, datatype TrainingInput, datatype TrainingOutput, std::vector<int> Dimensions)
{
	...
	for (int Cycles = 0; Cycles < CYCLELIMIT; Cycles++)
	{
		...
		{	
			...
			for (int i = 1; i < MatrixNum; i++)
			{
				...
				if (i == (MatrixNum - 1))
					DLayerVec[i] = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>::Zero(DLayerVec[i].rows(), DLayerVec[i].cols());
				else
		...
		{
			WGradVec[j] /= TrainingSize;
			BGradVec[j] /= TrainingSize;
		}

		//Performing Gradient Descent
		WeightVec = Gradient<std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>>>(WeightVec, WGradVec);
		BiasVec = Gradient<std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>>>(BiasVec, BGradVec);
	}
